...
#### 📥 데이터 백업 & 클라우드 동기화
- **JSON Export/Import**
  - 📥 현재사항 저장: 원클릭으로 JSON 백업 다운로드
  - 📤 데이터 가져오기: JSON 파일 업로드로 복원
  - 🔄 브라우저 간 데이터 이동
- **Cloud Sync (Firebase 연동 시)**
  - ☁️ Firebase Storage에 프로젝트 저장
  - 🆔 고유 Project ID로 공유 링크 생성
  - 🌐 다른 브라우저/기기에서 불러오기

#### 🔀 Service Flow (단계별 구성)
- **플로우 빌더**
  - 5가지 커스텀 노드: 시작, 인증, 핵심 로직(Vibe Code), 결제, 완료
  - New Day 스타일 카드 디자인
  - 세로 방향 자동 정렬 & 연결
- **상태 관리**
  - Zustand 스토어로 실시간 상태 관리
